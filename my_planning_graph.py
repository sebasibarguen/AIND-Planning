...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Create a set for each effect add or remove from each node. Do
        # set intersection operation to see if there is some overlap.
        test_a_b = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)
        test_b_a = set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)

        # Check if one of the two sets are non-empty
        if test_a_b or test_b_a:
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        test_a_b = (set(node_a1.action.precond_pos) & set(node_a2.action.effect_rem)) | \
                    (set(node_a1.action.precond_neg) & set(node_a2.action.effect_add))

        test_b_a = (set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) | \
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n1_parent in node_a1.parents:
            for n2_parent in node_a2.parents:
                if n1_parent.is_mutex(n2_parent):
                    return True
        return False
